...
// ═══════════════════════════════════════════════════════════════════════════════
// SESSION LOGGER  (with timers, warmup, resistance modes)
// ═══════════════════════════════════════════════════════════════════════════════
...
function SessionLogger({clients,sessions,onSave,activeClient,programs}){
  ...
  const triggerRest=sec=>{if(sec>0){setRestFor(sec);startRest(sec);}};
  ...
  const loadFromDay=day=>{
    ...
    setExercises(day.exercises.map(e=>({id:uid(),name:e.name,resistanceMode:"weighted",sets:[],templateSets:e.sets,templateReps:e.reps,templateRest:e.rest,notes:e.notes})));
    setLoadModal(false);
  };
  ...
  // PR detection using existing session history
  ...
  if(saved){
    ...
    return(
      <Card style={{textAlign:"center",padding:48}}>
        ...
        <div style={{color:C.sub,marginBottom:24}}>{totalSets} sets saved for {cl?.name}</div>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:16,paddingTop:settings.restTimerPosition==="top"&&restFor?restBarHeight:0,paddingBottom:settings.restTimerPosition==="bottom"&&restFor?restBarHeight:0}}>
      ...
      <Card>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
            ...
            <button onClick={()=>setShowSettings(true)} style={{width:38,height:38,borderRadius:8,background:C.s2,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:16,color:C.sub,flexShrink:0,marginBottom:0}}>⚙</button>
          </div>
        </div>
      ...
      {exercises.map(ex=>(
        <ExerciseCard key={ex.id} ex={ex} settings={settings}
          ...
          onUpdateSet={(exId,sid,field,val)=>{
            updateSet(exId,sid,field,val);
            // PR check when load or reps change on a done set
            const s=exercises.find(e=>e.id===exId)?.sets.find(s=>s.id===sid);
            if(s?.done){
              if(field==="load") checkPR(exId,sid,val,s.reps);
              if(field==="reps") checkPR(exId,sid,s.load,val);
            }
          }}
          onRemoveSet={removeSet} onRemove={removeEx} onRestStart={triggerRest}/>
      ))}

      {/* Add exercise */}
      <div style={{position:"relative"}}>
        <Btn variant="ghost" color={C.green} onClick={()=>setShowPicker(p=>!p)}>+ Add Exercise</Btn>
        {showPicker&&<ExPicker onPick={n=>{addEx(n);setShowPicker(false);}} onClose={()=>setShowPicker(false)}/>}
      </div>
